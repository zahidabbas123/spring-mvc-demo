...
<head>
	...
	<title>FLAT - Account_Group List</title>
...
<div class="breadcrumbs">
	<ul>
		...
		<li>
			<a href="tables-basic.html">Master</a>
			<i class="fa fa-angle-right"></i>
		</li>
		<li>
			<a href="tables-large.html">Account-Group List</a>
		</li>
	</ul>
...
<table class="table table-hover table-nomargin table-bordered dataTable dataTable-fixedcolumn dataTable-scroll-x ">
			...
			<tbody>  <%int count=0; %>
			    <c:forEach var="tempAccountGroup"  items="${accountGroupList}"      >
				     ...
				     <c:url var="UpdateLink"   value="/AccountGroup/showFormForUpdate" >
					   <c:param name="AccountGroupID" value="${tempAccountGroup.accountGroupId}" />
					 </c:url>
					 <c:url var="DetailsLink"   value="/AccountGroup/showFormForUpdate" >
					   <c:param name="AccountGroupID" value="${tempAccountGroup.accountGroupId}" />
					 </c:url>
					 <c:url var="DeleteLink"   value="/AccountGroup/deleteAccountGroup" >
					   <c:param name="AccountGroupID" value="${tempAccountGroup.accountGroupId}" />
					 </c:url>
				<tr>
					    <td>${count=count+1}</td>
						...
						|   <a href="${DeleteLink}"  
						          onclick="if (!(confirm('Are you sure you want to delete this Account-Group?'))) return false">Delete</a>
						|   <a href="${DetailsLink}" >Detail</a> 
						</td>
					</tr>
				</c:forEach>
			</tbody>
